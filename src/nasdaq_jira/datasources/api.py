...
class JiraApiDataSource(JiraDataSource):
    ...
    async def search_issues(self, jql: str) -> list[JiraIssue]:
        ...
        while True:
            response = await self._request(
                "GET", "/rest/api/2/search", params={"jql": jql, "startAt": start_at, "maxResults": self._config.page_size}
            )
    ...
    async def get_comments(self, issue_key: str) -> list[JiraComment]:
        ...
        return [JiraComment(body=item.get("body", ""), author=self._name(item.get("author")), created_at=item.get("created")) for item in response.json().get("comments", [])]
    ...
    async def _request(self, method: str, path: str, **kwargs: Any) -> httpx.Response:
        async def operation() -> httpx.Response:
            ...
            logger.info("Jira API %s %s latency=%.3fs", method, path, time.perf_counter() - started)
            if response.status_code in {401, 403}:
                raise AuthenticationError(f"Jira API authentication failed ({response.status_code})")
            ...
            return response
        return await retry_async(operation, attempts=self._config.retry_attempts, initial_delay=self._config.retry_initial_delay)
    ...
    def _issue_from_json(cls, payload: dict[str, Any]) -> JiraIssue:
        ...
        return JiraIssue(key=payload.get("key", ""), summary=fields.get("summary", ""), status=cls._name(fields.get("status")), priority=cls._name(fields.get("priority")), description=fields.get("description"), assignee=cls._name(fields.get("assignee")), reporter=cls._name(fields.get("reporter")), labels=fields.get("labels", []), components=[item.get("name", "") for item in fields.get("components", [])], fix_versions=[item.get("name", "") for item in fields.get("fixVersions", [])], updated_at=fields.get("updated"), source_url=payload.get("self"))